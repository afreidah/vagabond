...
WITH wanted AS (
    SELECT
        (@namespaces::text[])[g.i] AS namespace,
...
standing AS (
    SELECT
        ...
        COALESCE((
            ...
            WHERE u.namespace = wanted.namespace AND u.provider = @provider::text
              AND u.pool = wanted.pool AND u.period = wanted.period
        ), 0) + COALESCE((
            SELECT SUM(r.amount) FROM quota_reservations r
            WHERE r.namespace = wanted.namespace AND r.provider = @provider::text
...
SELECT @execution_id::text, s.namespace, @provider::text, s.pool, s.period, s.amount, @cpu::bigint, @memory::bigint, @created_at::timestamptz
FROM standing s
WHERE NOT EXISTS (
