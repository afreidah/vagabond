...
func TestStreamFailureDoesNotFailTheExecution(t *testing.T) {
	...
	outcome, err := d.RunTask(t.Context(), origin, containerTask(t, nil), nil, nil)
	if err != nil {
...
func TestProgressCarriesTheAttemptNumber(t *testing.T) {
	...
	if _, err := d.RunTask(t.Context(), origin, containerTask(t, reroutingRetry(2)), nil, nil); err != nil {
		t.Fatalf("RunTask failed: %v", err)
	}
